...
start = time()
m = Optimizer(dist_new, x0,args=(static,pca_moving,length,lnk_cost,500,1),method='L-BFGS-B',options=options)
end = time()

m.print_summary()

np.save('out/dist_link9.npy',m.xopt)
...
plt.plot(costs)
#plt.title("Cost Function\nTime: {:02}:{}:{}, Maxiter=200, lambda=1".format(hours,minutes,seconds))
plt.legend(['Distance','Link'])
plt.savefig("pics/cost_plot9.png",dpi=600)
